...
- Local brokers under `src/brokers/` (`paper_ashare` demo ledger; **`okx`** production-shaped CEX service). Market data under `src/marketdata/` (Fuyao/HiThink).
...
| `uv run okx` | OKX v5：status / balance / positions / ticker / order(place\|cancel\|amend\|get) / orders / fills / algo(sl\|tp\|oco\|place\|cancel) / algos；**服务层** `brokers.okx.OkxService`（量化复用）；默认模拟盘 + dry-run |
...
A-share Pi paper profile: `ashare-pi-paper` (local skills: paper-ashare-trade + investment-search; **npx project** skill: `HiThink-Tech/Financial-API` → `hithink-finance` into the **instance** `.agents/skills/`; profile `schedule/` with `context_files` @path inject). Init: `uv run harness instance init <name> --agent ashare-pi-paper`. Sync / launch runs `npx skills update -p -y` + `npx skills add … --yes` with cwd=instance (**never `-g`**).

A-share fundamental research (no trading): `ashare-grok-fundamental` on **Grok Build** harness (`configs/harnesses/grok-build/`). Auth + default model = host `~/.grok` membership (no workspace model override, no `GROK_HOME`). npx skill `mcncarl/yichen-skills` → `yichen-unified-search` into instance `.grok/skills/`; market data via `fuyao`; shared `memory/` + per-company/industry projects under `workdir/`. Init: `uv run harness instance init <name> --agent ashare-grok-fundamental`. Start with `./bin/grok` or `harness instance launch`.

OKX demo dual-MA (Grok + **grok-4.5**): profile `okx-grok-ma`. Pins `-m grok-4.5` on launch/schedule wake; `bin/okx` + `scripts/ma_scan.py` condition; schedule `scan-tick` (15m) / `research-tick` (1h). Init: `uv run harness instance init <name> --agent okx-grok-ma`. Demo keys in `.env` / `agent/secrets.env` (`OKX_SIMULATED=1`, often `OKX_HTTP_PROXY`). Example instance: `okx-grok-demo`.
...
- OKX keys: `OKX_API_KEY` / `OKX_API_SECRET` / `OKX_PASSPHRASE`；`OKX_SIMULATED=1`（默认模拟盘）。量化程序 import `OkxService`，不要只依赖 CLI。详见 `docs/okx.md`。
...
| **Profile** (many) | `configs/profiles/<id>/` | Experiment: `config.yaml` declares the profile harness and agent; `config.yaml` (`skills` / `bin` / `env` / `trade`); `skeleton/` (decision loop / memory — **no** Tools/Rules); **`schedule/`** |

Same harness can back many profiles.  
`uv run harness instance init <name> --agent <id>` merges harness⊕profile into the instance (self-contained `config.yaml` + skeleton copy; existing files skipped).

**Runtime sync:** `harness instance sync-settings` renders `materialize` templates. `sync-bin` writes `bin/` from profile `bin:` and appends a compact tool index to `agent_context`; profile rules remain in the skeleton. Launch/tick merge `env` + `process_env` (`process_env_policy: inherit|minimal`). Pi uses `minimal` + `PI_CODING_AGENT_DIR` + CLI `--no-*` so it does **not** use `~/.pi` / `~/.agents`; start via `./bin/pi` or `harness instance launch`, never bare `pi`. Profile `model: provider/id` expands into Pi `settings.json`.

**Agent bin (`bin:`):** each key → `instances/<name>/bin/<key>`. Optional `via` (default = key; `schedule` → `harness schedule`). Optional `enable`: list of regexes (`re.fullmatch`) selecting allowed subcommands — used both for the wrapper allow-list and for the compact generated Tools index (so `register` stays out of trading context). Omit `enable` = all subcommands / full passthrough.
...
- `docs/learn/` — 外部仓库对照笔记（源码 clone 在本机 `~/code/learn/<Name>/repo/`，不进本仓）
