...
Pi agent config is **instance-local** (`.pi/agent` via `PI_CODING_AGENT_DIR`). Launch also
passes `--no-extensions/--no-skills/...` so `~/.pi` and `~/.agents` are not loaded.
**Do not run bare `pi`** in the instance cwd — that ignores isolation.

```bash
# from repo root — requires AIPROXY_API_KEY in repo .env
uv run harness instance sync-settings <name>
uv run harness instance launch <name> --print
uv run harness instance launch <name>          # interactive
# or inside the instance after sync:
./bin/pi
```

Do **not** put register/launch/pi-env into the trading context file (`AGENTS.md`).
